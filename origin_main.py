...
if __name__ == "__main__":
    ...
    if MODE == 1:
        ...
        # 划分训练/测试
        # 注意：先做清洗&分词，再后续构造向量，然后才划分做性能对比
        text_for_chars = tokenized_text
        tag_for_chars = tag
    else:
        # 交叉数据集模式：用 TRAIN_DATA 训练，用 TEST_DATA 测试
        ...
        # 训练集用于统计汉字、构造模型
        text_for_chars = train_tokenized
    ...
    if os.path.exists(HANZI_CACHE):
        ...
        chinese_characters, chinese_characters_count, chinese_characters_code = count_chinese_characters(
            [''.join(s) for s in text_for_chars],  # 传入字符串列表
    ...
    char_vectors = generate_char_vectors(
        ...
        w2v_vectors,
        sim_mat,
        [''.join(s) for s in text_for_chars],
    ...
    if MODE == 1:
        ...
        # 交叉数据集：分别为 train 和 test 单独生成句向量
        ...
        train_vectors, test_vectors, train_tag, test_tag = train_sentence_vectors, test_sentence_vectors, train_tag, test_tag
